                ...
                playerStatsKeyIndex_t divisionParam)
{
    ...
    char backupBuffer[LIVE_COMPRESSED_CAC_SIZE]; // [esp+94h] [ebp-9D20h] BYREF
    ...
    if ( index < 256 )
    {
        ...
        if ( forOtherPlayer )
        {
            ...
            if ( LiveStats_GetStatsDDL() && LiveStats_GetStatsDDL()->version != HeaderVersion )
            {
                ...
                if ( !DDL_FixBufferVersion(buffer, StatsDDL, "ddl_mp/stats_archive.ddl", backupBuffer, LIVE_MAX_CAC_SIZE) )
                {
                    v8 = LiveStats_GetStatsDDL();
                    if ( !DDL_FixBufferVersion(buffer, v8, "ddl_mp/stats.ddl", backupBuffer, LIVE_MAX_CAC_SIZE) )
                    {
